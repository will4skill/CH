...
router.get('/', async function(req, res){
  ...
  if ( protocol[current][next] == name_type || protocol[current][next] == "B" ) {
    try {
      const path = "https://warm-journey-29261.herokuapp.com/api/services/";
      const result = await axios.get(`${path}${req.query.new_service}/?name=${req.query.name}`);
...
router.get('/first-state', async function(req, res){
  ...
  const path = "https://warm-journey-29261.herokuapp.com/api/services/";
  const result = await axios.get(`${path}s${starting_service}/?name=${req.query.name}`);
